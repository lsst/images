...
class OutputArchive[P](ABC):
    ...
    def serialize_direct[T: pydantic.BaseModel | None](
        ...
        serializer
            Callable that takes an `~lsst.images.serialization.OutputArchive`
            and returns a Pydantic model.  This will be passed a new
            `~lsst.images.serialization.OutputArchive` that automatically
            prepends ``{name}/`` (and any root path added by this archive) to
            names passed to it, so the ``serializer`` does not need to know
            where it appears in the overall tree.
    ...
    def serialize_pointer[T: ArchiveTree](
        ...
        # the `~lsst.images.serialization.OutputArchive` opts to stuff all
        # pointer serializations into a standard location outside the
        # user-controlled Pydantic model tree, and always returned a JSON
        # pointer to that standard location from this function.
        raise NotImplementedError()
...
class NestedOutputArchive[P: pydantic.BaseModel](OutputArchive[P]):
    ...
    `~lsst.images.serialization.OutputArchive.serialize_direct` and
