...
class ZarrRoundTripTestCase(unittest.TestCase):
    """Full write -> read round-trips for the supported image types."""
    ...
    def test_mask_round_trip(self) -> None:
        ...
        with RoundtripZarr(self, original) as roundtrip:
            ...
            np.testing.assert_array_equal(recovered.array, original.array)
            self.assertEqual(recovered.bbox, original.bbox)
            self.assertEqual(list(recovered.schema.names), list(original.schema.names))

    def test_uint16_mask_packs_with_element_stride(self) -> None:
        ...
        with RoundtripZarr(self, original) as roundtrip:
            with open_store_for_read(roundtrip.filename) as store:
                ...
                self.assertEqual(flag_masks[16], 1 << 16)
                self.assertNotEqual(on_disk & flag_masks[16], 0)
            recovered = roundtrip.result
            np.testing.assert_array_equal(recovered.mask.array, original.mask.array)
