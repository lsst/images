...
### Task 6: `SkyProjection` pixel-scale methods and `_describe(bbox=None)` (tabular, KAPPA-style)

**Files:**
- Modify: `python/lsst/images/_transforms/_sky_projection.py` (`SkyProjection`, lines ~55-97) — adds `_nominal_pixel_scale`, `_pixel_axis_report`, and `_describe`
...
- Produces two independent, independently-tested pixel-scale methods and `_describe`:
  - `SkyProjection._nominal_pixel_scale(self, bbox: Box) -> list[float]`: the nominal pixel scale in arcsec/pixel **per sky axis** (`[longitude, latitude]`), a faithful port of Starlink KAPPA `KPG1_SCALE`/`KPG1_PXSCL` (written by the AST author). This is the "sky-axis dominance" view: under rotation the RA scale attaches to whichever pixel axis maps to RA, so an unrotated `0.2×0.3` WCS reports `[0.2, 0.3]` and the same WCS rotated 90° reports `[0.3, 0.2]`. Correct near the poles and under rotation.
  - `SkyProjection._pixel_axis_report(self, bbox: Box) -> list[tuple[float, str, str, bool]]`: per **pixel** axis (x then y), a `(scale_arcsec, label, units, diagonal)` tuple where `label`/`units` name the sky direction that pixel axis predominantly tracks and `diagonal` flags an axis running near 45° to both. This is the "pixel-axis dominance" view: the scale stays attached to its pixel axis while the label follows the sky direction, so a 90° rotation reads Axis `x` → "Declination", Axis `y` → "Right ascension".
  - Both are private for now and candidates for promotion later. Keeping both means the "Axes" table can be reframed (sky-axis vs pixel-axis) by swapping which one `_describe` calls, with no change to the underlying math.
  - `SkyProjection._describe(self, *, bbox: Box | None = None, **kwargs) -> Report`. `SkyProjection` gains `DescribableMixin` as a base. The report has:
    ...
    - A `DERIVED` `ReportTable` titled `"Axes"` (columns `["Axis", "Label", "Units", "Nominal pixel scale"]`). **For now it uses the pixel-axis view** (`_pixel_axis_report`): **one row per pixel axis** (`"x"`, `"y"`) with the label/units/scale from that method; a near-diagonal axis appends `" (diagonal)"` to its label. Scales are populated only when a bbox is available (otherwise the label/units default to the unrotated `x=RA, y=Dec` convention and the scale is `"-"`).
    - A `DERIVED` `ReportTable` titled `"Corners"` (columns `["Corner", "RA", "Dec"]`), only when a bbox is available.

#### The two pixel-scale algorithms

Both descend from Starlink KAPPA `KPG1_DSFRM` → `KPG1_SCALE` → `KPG1_PXSCL` in KAPLIBS and share the two robust ideas that make KAPPA correct near poles and under rotation:

- **Great-circle distance** (`SkyCoord.separation`, the astropy analogue of AST's `AST_DISTANCE` on a sky frame) for every sky step — never a naive dRA/dpix.
...
They differ only in what a "scale" is attributed to:

1. **`_nominal_pixel_scale` — per sky axis (faithful `KPG1_PXSCL` port).** At each test point, transform the point and all eight unit-offset neighbours (`{-1,0,+1}²` minus the center) to the sky. For each sky axis, pick the neighbour whose transformed position moves *farthest* along that axis; build a probe sky coordinate that differs from the center only on that axis; take the great-circle distance to it divided by the pixel-space (Euclidean) distance between the center pixel and the probe transformed back through `sky_to_pixel` (rounding the center pixel back through `sky_to_pixel` too, to keep the grid reference consistent — the `PermMap` safety in the original). Median per sky axis.
2. **`_pixel_axis_report` — per pixel axis.** Perturb directly along each pixel axis (unit step `(1,0)` for x, `(0,1)` for y); `scale = center.separation(step).to_value(arcsec)`, kept with that pixel axis. The label is the dominant sky direction: compare the RA component `|Δra·cos(dec)|` against the Dec component `|Δdec|`; the larger names the axis, and when they are comparable (`min/max > 0.8`) flag the axis `diagonal`. Median per pixel axis.

Do not simplify either to a single adjacent-pixel `separation`; that heuristic is what these deliberately replace.

- [ ] **Step 1: Write the failing test for the per-sky-axis scale (`_nominal_pixel_scale`)**
...
- [ ] **Step 3: Implement the per-sky-axis scale method**
...
Run: `.pyenv/bin/pytest tests/test_transforms.py::test_sky_projection_nominal_pixel_scale -v`
Expected: PASS (unrotated `[0.2, 0.3]`, rotated 90 deg `[0.3, 0.2]`, near-pole `[0.2, 0.2]`).

- [ ] **Step 5: Write the failing test for the pixel-axis report (`_pixel_axis_report`)**

Add to `tests/test_transforms.py` (`_rotated_tan` was defined in Step 1; reuse it):
...
- [ ] **Step 7: Implement the pixel-axis report method**

Add this method to the class alongside `_nominal_pixel_scale` (the imports it needs were added in Step 3):
...
- [ ] **Step 9: Write the failing test for `_describe`**
...
- [ ] **Step 11: Adopt the mixin and implement `_describe`**
...
Run: `.pyenv/bin/pytest tests/test_transforms.py::test_sky_projection_describe tests/test_transforms.py::test_sky_projection_pixel_axis_report tests/test_transforms.py::test_sky_projection_nominal_pixel_scale -v`
Expected: PASS.

- [ ] **Step 13: Confirm no existing SkyProjection tests regressed**
...
git commit -m "Add KAPPA describe report and pixel-scale methods to SkyProjection

Add both a faithful per-sky-axis KPG1_PXSCL port (_nominal_pixel_scale)
and a per-pixel-axis report (_pixel_axis_report); the Axes table uses the
pixel-axis view for now, leaving the sky-vs-pixel framing swappable.
